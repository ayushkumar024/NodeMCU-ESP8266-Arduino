...
#include <ESP8266WiFi.h>
 ...
 /*------------------------------------------------------------
     ...
     *-------------------------------------------------------------*/   
     char NAME[]="<adafruit_userName>";
     char API_KEY[]="<adafruit_api>";
  ...
  /*-----------------------------------------------------------
     ...
     *------------------------------------------------------------*/
   Adafruit_MQTT_Publish mypub=Adafruit_MQTT_Publish(&mqtt,"Ayush098/feeds/status");
   Adafruit_MQTT_Subscribe mysub=Adafruit_MQTT_Subscribe(&mqtt,"Ayush098/feeds/onoff");

void setup() {
  // put your setup code here, to run once:
  pinMode(5,OUTPUT);
  WiFi.begin("vivo","qwerty123");
  Serial.begin(115200);
   ...
   else if(mqtt.connect()==0)
   {
      Serial.println("Connecting to the MQTT....");    
             ...
             {  
                if(sub==&mysub)
                {
                  analogWrite(5,atoi((char *)mysub.lastread));
